...
class Client:
    ...
    def __init__(self, params: dict):
        ...
        self.query_params: list = ['offset', 'limit', 'sort', 'q']
    ...
    def get_indicator(self, indicator_value: str, indicator_type: str) -> dict:
        args: dict = {
...
def get_score_from_resource(r: dict) -> int:
    """
...
def get_indicator_object(indicator_value: Any, indicator_type: str, dbot_score: Common.DBotScore,
                         extra_context: dict = None) -> Union[Common.IP, Common.URL, Common.File, Common.Domain, None]:
    """
...
def build_indicator(indicator_value: str, indicator_type: str, title: str, client: Client) -> CommandResults:
    ...
    """
    res: dict = client.get_indicator(indicator_value, indicator_type)
...
def get_values(items_list: list, return_type: str = 'str', keys: Union[str, list] = 'value') -> Union[str, list]:
    """
...
def get_indicator_data(o: dict) -> Tuple[dict, dict]:
    """
    ...
    """
    output: dict = dict()
    indicator_context_fields: dict = dict()
    ...
    if o:
        ...
        relations: list = o.get('relations', [])[:10]
        labels: list = o.get('labels', [])[:10]

        output = assign_params(**{
            'ID': indicator_id,
            'Type': indicator_type,
            'Value': indicator_value,
            'LastUpdate': timestamp_to_datestring(last_update) if last_update else None,
            'PublishDate': timestamp_to_datestring(publish_date) if publish_date else None,
...
def test_module(client: Client) -> Union[str, Exception]:
    ...
    """
    output: dict = client.check_quota_status()
...
def cs_actors_command(client: Client, args: dict) -> CommandResults:
    res: dict = client.cs_actors(args)
    resources: list = res.get('resources', [])
    outputs: list = list()
    md: str = str()
    ...
    if resources:
        for r in resources:
            ...
            output: dict = assign_params(**{
                ...
                'FirstActivityDate': timestamp_to_datestring(first_activity_date) if first_activity_date else None,
                'LastActivityDate': timestamp_to_datestring(last_activity_date) if last_activity_date else None,
                'Active': active,
                'KnownAs': known_as,
...
def cs_indicators_command(client: Client, args: dict) -> CommandResults:
    res: dict = client.cs_indicators(args)
    resources: list = res.get('resources', [])
    indicators: list = list()
    outputs: list = list()
    md: str = str()
...
def cs_reports_command(client: Client, args: dict) -> CommandResults:
    res: dict = client.cs_reports(args)
    resources: list = res.get('resources', [])
    outputs: list = list()
    md: str = str()
    ...
    if resources:
        for r in resources:
            ...
            output: dict = assign_params(**{
                ...
                'SubType': sub_type,
                'Slug': slug,
                'CreatedDate': timestamp_to_datestring(created_date) if created_date else None,
                'LastModifiedSate': timestamp_to_datestring(last_modified_date) if last_modified_date else None,
                'ShortDescription': short_description,
                'TargetIndustries': get_values(target_industries, return_type='list'),
                'TargetCountries': get_values(target_countries, return_type='list'),
                'Motivations': get_values(motivations, return_type='list'),
